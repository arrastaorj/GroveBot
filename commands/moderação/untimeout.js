...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (!interaction.member.permissions.has(discord.PermissionsBitField.Flags.ModerateMembers)) {
            return interaction.reply({ content: "🚫 Você não tem permissão para remover timeout de membros.", ephemeral: true });
        }
        ...
        if (!member.communicationDisabledUntil) {
            return interaction.reply({ content: "⚠️ Este usuário não está atualmente em timeout.", ephemeral: true });
        }
        ...
        try {
            ...
            console.error(error);
            return interaction.reply({ content: "❗ Ocorreu um erro ao tentar remover o timeout deste usuário.", ephemeral: true });
        }
    }
};
