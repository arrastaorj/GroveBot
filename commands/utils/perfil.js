...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        let foto = db.get(`skins${interaction.guild.id}_${user.id}`)

        if (foto === null) foto = "https://cdn.discordapp.com/attachments/1063231058407079946/1063642572892934186/file222.png"
        else { foto = foto.attachment }
        ...
        let lvl = db.fetch(`lvl_${interaction.guild.id}_${user.id}`)
        if (lvl == null) lvl = 0
        chave.context.font = '50px "up"'
        ...
        let rep = db.fetch(`rep${interaction.guild.id}_${user.id}`)
        if (rep == null) rep = 0
        ...
        if (interaction.user != user) {
            ...
            collector.on('collect', async (i) => {
                ...
                if (i.customId === 'confirma') {
                    ...
                    const attachment = createAttachmentBuilder("https://cdn.discordapp.com/attachments/1063231058407079946/1065445574100398202/minecrafttt.png", `${interaction.user.tag}.png`)
                    await i.reply({ content: `Skin selecionada com sucesso. Aproveite!`, ephemeral: true })
                    await db.set(`skins${interaction.guild.id}_${user.id}`, attachment)

                }

                if (i.customId === 'confirma2') {
                    if (!i.isButton()) return
                    const attachment = createAttachmentBuilder("https://cdn.discordapp.com/attachments/1063231058407079946/1065442957827784814/settt.png", `${interaction.user.tag}.png`)
                    await db.set(`skins${interaction.guild.id}_${user.id}`, attachment)
                    await i.reply({ content: `Skin selecionada com sucesso. Aproveite!`, ephemeral: true })
                ...
                if (i.customId === 'confirma3') {
                    if (!i.isButton()) return
                    const attachment = createAttachmentBuilder("https://cdn.discordapp.com/attachments/1063231058407079946/1065441816985481246/vaynearcoceleste.png", `${interaction.user.tag}.png`)
                    await db.set(`skins${interaction.guild.id}_${user.id}`, attachment)
                    await i.reply({ content: `Skin selecionada com sucesso. Aproveite!`, ephemeral: true })
